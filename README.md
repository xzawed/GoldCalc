...
현재 코드는 `Header` + `PriceBar`(현재 금시세/환율 표시) + 오프라인 배너까지 동작하고,
3개 본 섹션은 Sprint 계획대로 플레이스홀더로 존재합니다.

- **구현됨**: `Header`, `PriceBar`(GoldAPI 기반 현재가/환율), `OfflineBanner`, 공통 유틸(`format.ts`) 및 테스트 인프라(MSW + Vitest 설정)
...
- `src/utils/goldCalc.test.ts` — 단위 테스트 (커버리지 100% 목표)
...
- `src/utils/historyCalc.test.ts` — 단위 테스트
...
- `src/utils/forecast.test.ts` — 단위 테스트

...
### 4구역 — 금융 소식 (최하단, 모든 탭 공통)
13. **X(Twitter) 임베드 타임라인**: 금시세·환율·거시경제 관련 X 리스트 타임라인
    - `widgets.js` 기반 무료 임베드 — API 키 불필요
    - 다크 테마, 헤더/푸터 없는 투명 스타일
    - 로딩 중 스켈레톤 표시, 네트워크 차단 시 fallback + 직접 링크
    - 임베드 URL: `src/constants/newsDefaults.ts`의 `DEFAULT_X_EMBED_URL` 수정으로 변경
    - 추천 계정 목록(MAGA/금융 인증 계정): `RECOMMENDED_X_ACCOUNTS` 참고
...
│  │  (widgets.js, 다크 테마, 무료)   │    │
│  └─────────────────────────────────┘    │
│  DEFAULT_X_EMBED_URL 수정으로 소스 변경  │
└─────────────────────────────────────────┘
```
...
│   │   └── news/
│   │       └── NewsSection.tsx       # 4구역 X(Twitter) 금융 소식 임베드 (모든 탭 공통)
│   ├── hooks/
...
│   ├── constants/
│   │   └── newsDefaults.ts          # X 임베드 URL·위젯 옵션·추천 계정 목록
│   ├── utils/
